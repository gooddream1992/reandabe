					...
					<?php if($med==$size){
						...
						<div class="col-sm-6">
							<div class="nav-services flex-box" style="margin:1.5px">
								<a href="{{ route('compro.servListId', ['id'=> $services[0]->service_id]) }}" class="btnc-more" style="padding:5px 15px;margin:inherit">{{ $services[0]->name_ch}}</a>
							</div>
						</div>
					...
					<?php }else {?>
						<div class="col-sm-6">
							<div class="nav-services flex-box" style="margin:1.5px">
								<?php for($i=0;$i<$med;$i++){?>
								@if ( Session::get('language') == 'ch' )
									<a href="{{ route('compro.servListId', ['id'=> $services[$i]->service_id]) }}" class="btnc-more" style="padding:5px 15px;margin:inherit">{{ $services[$i]->name_ch}}</a>
								@elseif ( Session::get('language') == 'en'  )
						...
						<div class="col-sm-6">
							<div class="nav-services flex-box" style="margin:1.5px">
								<?php for($j=$med;$j<$size;$j++){?>
								    @if ( Session::get('language') == 'ch' )
									<a href="{{ route('compro.servListId', ['id'=> $services[$j]->service_id]) }}" class="btnc-more" style="padding:5px 15px;margin:inherit">{{ $services[$j]->name_ch}}</a>
								@elseif ( Session::get('language') == 'en'  )
